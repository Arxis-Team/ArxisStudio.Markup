...
Never add to these packages: MSBuild evaluation, `.sln`/`.csproj`/`project.assets.json` reading, NuGet search or restore, C# compilation, Roslyn analysis, IDE integration, a visual designer, selection adorners, property-inspector UI, drag and drop, pointer/keyboard interception, or a sandbox for untrusted XAML. This governs `src/`; a sample may demonstrate what a host builds on the published API — the showcase has a property inspector for exactly that reason, recorded in `docs/adr/0006-inspector-in-the-sample.md`. External environments enter only through the resolver/provider interfaces (`IMarkupSourceProvider`, `IXamlAssemblyResolver`, `IXamlTypeResolver`, `IXamlResourceResolver`, `IXamlRootInstanceFactory`, `IXamlDispatcher`) — the contract calls the first of those `IXamlSourceProvider`, see `docs/adr/0004-loader-boundaries.md`. Do not create placeholder implementations for out-of-scope features. One package is allowed nearer this line than the rest: `ArxisStudio.Markup.Xaml.Design` exists to make a `TopLevel`-rooted document viewable, because a `Window` cannot be the content of anything and so the object this loader correctly produces for the commonest document in the ecosystem cannot be shown. It may host; it may not select. No adorner, no handle, no pointer or keyboard handler, no inspector — if it ever needs to know what is selected, the code belongs to a host or an editor and not here. See `docs/adr/0012-hosting-a-top-level-root-is-a-package-beside-the-loader.md`.
